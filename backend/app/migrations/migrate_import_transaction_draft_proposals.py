"""Module for backend app migrations migrate_import_transaction_draft_proposals."""
...
def migrate_import_transaction_draft_proposals(
    bind_engine: Engine = default_engine,
) -> None:
    """Handle migrate import transaction draft proposals."""
    ...
    if "import_transaction_drafts" not in inspector.get_table_names():
        logger.info(
            "import_transaction_drafts table does not exist; skipping migration"
        )
        return
    ...
    with bind_engine.begin() as conn:
        for column_name, statement in column_statements.items():
            if column_name not in existing_columns:
                ...
                logger.info(
                    "Added missing import transaction draft column: %s", column_name
                )
