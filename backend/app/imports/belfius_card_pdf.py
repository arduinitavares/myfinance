...
PERIOD_RE = re.compile(r"Transacties van\s+(\d{2}/\d{2}/\d{4})\s+tot\s+(\d{2}/\d{2}/\d{4})", re.IGNORECASE)
CARD_RE = re.compile(r"Kaartnummer\s+([0-9X ]{19,})", re.IGNORECASE)
FX_RATE_LINE_RE = re.compile(r"^\d+(?:,\d+)?(?:\.\d+)?[A-Z]{3}$")
INLINE_FX_HELPER_RE = re.compile(r"\s+1 EUR =$")
...
def _infer_transaction_date(day_month: str, statement_start: str, statement_end: str) -> str:
...
class BelfiusCardPdfExtractor:
    ...
    def extract_from_pages(self, pages: list[dict], raw_artifact_ref: str) -> ExtractionResult:
        ...
        for page in pages:
            if not self._page_has_transaction_rows(page):
                continue
            transactions.extend(self._parse_transaction_page(page, statement_metadata, issues))
        ...
        for page in pages:
            for line in page["lines"]:
                ...
                if "statement_period_start" not in metadata:
                    ...
                    if period_match:
                        metadata["statement_period_start"] = _to_iso_date(period_match.group(1))
                        metadata["statement_period_end"] = _to_iso_date(period_match.group(2))

                if "card_number_hint" not in metadata:
                    card_match = CARD_RE.search(text)
                    if card_match:
                        metadata["card_number_hint"] = _collapse_spaces(card_match.group(1))
    ...
    def _parse_transaction_page(
        ...
        for line in page["lines"]:
            ...
            if row_match:
                ...
                try:
                    signed_amount, debit_credit = _parse_eur_amount(row_match.group("eur_amount"), row_match.group("sign"))
                except ValueError:
    ...
    def _unclassifiable_issue(page_number: int, line_number: int, text: str) -> ImportIssue:
        return ImportIssue(
            ...
            message=f"Unclassifiable Belfius card line on page {page_number}, line {line_number}: {text}",
            blocking=True,
